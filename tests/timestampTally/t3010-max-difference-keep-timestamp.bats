...
@test "close RFC 3339 timestamps within 1 minute as third field explicitly specified and single entry duration of 1 minute with kept start timestamp" {
    run timestampTally --timestamp-field 3-4 --keep-timestamp start --max-difference 1m --single-entry-duration 1m <<<"${delayedMixedDates//T/ }"

    [ $status -eq 0 ]
    [ "$output" = "1593871643 foo 2020-07-04 16:07:23,001 2
...
@test "close RFC 3339 timestamps within 1 minute as third field explicitly specified and single entry duration of 1 minute with kept end timestamp" {
    run timestampTally --timestamp-field 3-4 --keep-timestamp end --max-difference 1m --single-entry-duration 1m <<<"${delayedMixedDates//T/ }"

    [ $status -eq 0 ]
    [ "$output" = "1593871645 foo3 2020-07-04 16:07:25,001 2
1593871800 bar 2020-07-04 16:10:00,011 60
...
@test "close RFC 3339 timestamps within 1 minute as third field explicitly specified and single entry duration of 1 minute with kept both concatenated timestamps" {
    run timestampTally --timestamp-field 3-4 --keep-timestamp both-concatenated --max-difference 1m --single-entry-duration 1m <<<"${delayedMixedDates//T/ }"

    [ $status -eq 0 ]
    [ "$output" = "1593871643 foo 2020-07-04 16:07:23,001 2020-07-04 16:07:25,001 2
...
@test "close RFC 3339 timestamps within 1 minute as third field explicitly specified and single entry duration of 1 minute with kept separate start and end timestamps" {
    run timestampTally --timestamp-field 3-4 --keep-timestamp both-separate --max-difference 1m --single-entry-duration 1m <<<"${delayedMixedDates//T/ }"
