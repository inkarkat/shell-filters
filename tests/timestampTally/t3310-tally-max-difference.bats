...
@test "ISO 8601 timestamps in field 2 with identical field 1 within 1 minute are condensed to the first occurrence" {
    run -0 timestampTally --timestamp-field 2 --max-difference 1m --single-entry-duration 7 --tally 1 <<<"$input"
    assert_output - <<'EOF'
foo 120 some
foo 7 too
bar 59.997 dense
...
@test "ISO 8601 timestamps in field 2 with identical field 1 within 1 minute are condensed to the first occurrence with kept start timestamp" {
    run -0 timestampTally --timestamp-field 2 --keep-timestamp start --max-difference 1m --single-entry-duration 7 --tally 1 <<<"$input"
    assert_output - <<'EOF'
foo 2020-07-04T16:07:23,001 120 some
...
@test "ISO 8601 timestamps in field 2 with identical field 1 within 1 minute are condensed to the first occurrence with kept end timestamp" {
    run -0 timestampTally --timestamp-field 2 --keep-timestamp end --max-difference 1m --single-entry-duration 7 --tally 1 <<<"$input"
    assert_output - <<'EOF'
...
@test "ISO 8601 timestamps in field 2 with identical field 1 within 1 minute are condensed to the first occurrence with kept both concatenated timestamps" {
    run -0 timestampTally --timestamp-field 2 --keep-timestamp both-concatenated --max-difference 1m --single-entry-duration 7 --tally 1 <<<"$input"
    assert_output - <<'EOF'
foo 2020-07-04T16:07:23,001 2020-07-04T16:09:23,001 120 some
...
@test "ISO 8601 timestamps in field 2 with identical field 1 within 1 minute are condensed to the first occurrence with kept separate start and end timestamps" {
    run -0 timestampTally --timestamp-field 2 --keep-timestamp both-separate --max-difference 1m --single-entry-duration 7 --tally 1 <<<"$input"
    assert_output - <<'EOF'
foo 2020-07-04T16:07:23,001 120 some
foo 2020-07-04T16:09:23,001 120 anyway
foo 2020-07-04T16:10:23,002 7 too
...
@test "ISO 8601 timestamps in field 2 with identical field 1 within 1 minute and a maximum duration of 80 seconds condensed to the first occurrence with kept separate start and end timestamps" {
    run -0 timestampTally --timestamp-field 2 --keep-timestamp both-separate --max-difference 1m --max-record-duration 80 --tally 1 <<<"$input"
    assert_output - <<'EOF'
foo 2020-07-04T16:07:23,001 60 some
...
@test "ISO 8601 timestamps in field 2 with identical field 1 within 1 minute and a maximum length of 2 are condensed to the first occurrence with kept separate start and end timestamps" {
    run -0 timestampTally --timestamp-field 2 --keep-timestamp both-separate --max-difference 1m --max-record-length 2 --tally 1 <<<"$input"
    assert_output - <<'EOF'
foo 2020-07-04T16:07:23,001 0.001 some
