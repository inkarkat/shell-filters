...
@test "duplicate interactive flash of accumulate in multiple locations" {
    run -0 collapseDuplicates --unbuffered --as flash --accumulate 'repeat' <<-'EOF'
This will repeat.
This is the repeat.
...
EOF
    assert_output - <<EOF
This will repeat.${LF}${R}This is the repeat.${N}${LF}This is the repeat.${LF}${R}That is the last repeat in the first location.${N}${LF}That is the last repeat in the first location.
A unique statement.
End of interlude.
