...
@test "matches with three different priorities in a line are highlighted" {
    run -0 extractMatches --regexp fo+ --global --priority 10 --regexp 'e\w' --priority 1 --regexp 'y' --global --priority 2 <<-'EOF'
Just some sexy text.
...
@test "matches with negative and positive priorities and a default priority in a line are highlighted" {
    run -0 extractMatches --regexp fo+ --global --priority 1 --regexp 'e\w' --priority -1 --regexp 'y' --global <<-'EOF'
Just some sexy text.
This has foo, foo and foofoo in it.
All simple lines.
More foo here.
...
@test "counts with negative and positive priorities and a default priority in a line are highlighted" {
    run -0 extractMatches --match-count fo+ --global --priority 1 --count 'e\w' --priority -1 --count 'y' --global <<-'EOF'
Just some sexy text.
This has foo, foo and foofoo in it.
All simple lines.
More foooo here.
