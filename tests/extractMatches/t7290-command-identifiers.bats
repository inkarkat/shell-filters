...
@test "include FILENAME with stdin in the command invocation" {
    ...
    run -0 extractMatches --to command --regexp fo+ --count '\<\w{4}\>' --global --match-count '\<i\w\>' --global <<<"$DELAY_INPUT"
...
@test "include FILENAME with an actual file in the command invocation" {
    ...
    run -0 extractMatches --to command --regexp fo+ --count '\<\w{4}\>' --global --match-count '\<i\w\>' --global "$TEST_FILE"
