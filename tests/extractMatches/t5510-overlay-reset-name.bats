...
@test "match resets by name are overlaid" {
    ...
    run extractMatches --to overlay --reset-name All --name fours --regexp '\<\w{4}\>' --name fours <<<"$SIMPLE_INPUT"
...
@test "count resets by name are overlaid" {
    ...
    run extractMatches --to overlay --reset-name All --name fours --count '\<\w{4}\>' --name fours <<<"$SIMPLE_INPUT"
    [ "$output" = "Just some text.
...
@test "match-count resets by name are overlaid" {
    ...
    run extractMatches --to overlay --reset-name All --name fours --match-count '\<\w{4}\>' --name fours <<<"$SIMPLE_INPUT"
    [ "$output" = "Just some text.
${R}Just:1${N}This has foo in it.
