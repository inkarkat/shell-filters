...
@test "another counts is reset by reset-pattern" {
    run -0 extractMatches --count fo+ --global --reset All --reset-other '\<text\>' --count '\<text\>' --global <<-'EOF'
Or some sexy text.
...
@test "two other counts are reset by pattern and reset-pattern of a third count" {
    run -0 extractMatches --count fo+ --global --count '\<[Oo]r\>' --global --reset-other fo+ --reset All --reset-other '\<text\>' --count '\<text\>' --global <<-'EOF'
Or some sexy text.
