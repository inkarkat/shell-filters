...
@test "three different counts with different single / global are written with percentages to a file every 3 lines and at the end" {
    run extractMatches --name-percentages '/^foo|^ex/' --name-percentages single --to "$LOG" --count 'foo[0-9]+' --global --count 'ex' --global --count 'y' --global --name single --count 'l' --global --name single <<<"$DELAY_INPUT"
    [ "$output" = "$DELAY_INPUT" ]
