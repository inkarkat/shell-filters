...
@test "match resets are written to a file" {
    run -0 extractMatches --to "$LOG" --regexp '\<\w{4}\>' --reset All <<<"$SIMPLE_INPUT"
...
@test "count resets are written to a file" {
    run -0 extractMatches --to "$LOG" --count '\<\w{4}\>' --reset All <<<"$SIMPLE_INPUT"
    assert_output "$SIMPLE_INPUT"
    assert_log "Just: 1
This: 2
This: 0
...
@test "match-count resets are written to a file" {
    run -0 extractMatches --to "$LOG" --match-count '\<\w{4}\>' --reset All <<<"$SIMPLE_INPUT"
    assert_output "$SIMPLE_INPUT"
    assert_log "Just: 1
This: 1
Just: 0
