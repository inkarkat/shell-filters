...
@test "prefix replacement of match is passed to command" {
    run -0 extractMatches --to command --regexp fo+ --replacement "FOO: &" <<<"$input"
    assert_output "$input"
    assert_runs "FOO: foo
FOO: fooo"
}
...
@test "capture group replacement of match and count is passed to command" {
    run -0 extractMatches --to command --regexp '(f)(o+)' --replacement "\1-\2" --count '\<(\w)\w{2}(\w)\>' --replacement "\1-\2" <<<"$input"
    assert_output "$input"
...
@test "plain replacements of count does not affect counting, only passing to command" {
    run -0 extractMatches --to command --count fo+ --global --replacement "FOO" --count 'i\w' --global --replacement "FOO" <<<"$input"
    assert_output "$input"
...
More fooo here. moooo moo"
    run -0 extractMatches --to command --match-count '([fm])(o)+' --global --replacement "\1\2" <<<"$input"
    assert_output "$input"
