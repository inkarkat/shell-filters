...
@test "single counts in a line are reset via name by another line" {
    run -0 extractMatches --count fo+ --name foos --reset-name All --name foos <<-'EOF'
Just some text.
This has foo in it.
All simple lines.
More foooo here.
...
@test "counts within a line are reset by an intermediate reset pattern that comes first" {
    run -0 extractMatches --reset-name and --name foos --count fo+ --global --name foos <<-'EOF'
...
@test "two different counts are reset" {
    run -0 extractMatches --match-count fo+ --global --name foos --reset-name '[Aa]nd' --name foos --match-count '\<\w{4}\>' --global --name fours --reset-name '[Oo]r' --name fours <<-'EOF'
Or some sexy text.
...
@test "count and match-counts are reset" {
    run -0 extractMatches --count fo+ --global --name foos --reset-name '[Aa]nd' --name foos --match-count '\<\w{4}\>' --global --name fours --reset-name '[Oo]r' --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foo and foofoo in it.
All more simple text.
...
@test "earlier counts consume those given later" {
    run -0 extractMatches --match-count fo+ --global --match-count oo --global --name double-o --reset-name and --name double-o --match-count o+ --global --match-count 'ex' --match-count 'x' --global <<-'EOF'
Just some sexy text.
