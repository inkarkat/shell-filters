...
@test "single counts in a line are reset by another line" {
    run -0 extractMatches --count fo+ --reset All <<-'EOF'
Just some text.
This has foo in it.
All simple lines.
More foooo here.
...
@test "counts within a line are reset by an intermediate reset pattern" {
    run -0 extractMatches --count fo+ --global --reset and <<-'EOF'
...
@test "two different counts in a line are highlighted" {
    run -0 extractMatches --match-count fo+ --global --reset '[Aa]nd' --match-count '\<\w{4}\>' --global --reset '[Oo]r' <<-'EOF'
Or some sexy text.
...
@test "earlier counts consume those given later" {
    run -0 extractMatches --match-count fo+ --global --match-count oo --global --reset and --match-count o+ --global --match-count 'ex' --match-count 'x' --global <<-'EOF'
Just some sexy text.
