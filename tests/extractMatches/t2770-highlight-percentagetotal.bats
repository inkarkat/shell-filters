...
@test "percentages against total for a single count" {
    run extractMatches --percentage-total foos 5 --count fo+ --global --name foos <<-'EOF'
This has foo, foo and foofoo in it.
More foooo and foo here.
EOF
    [ "$output" = "This has [foo (1 = 20%)], [foo (2 = 40%)] and [foo (3 = 60%)][foo (4 = 80%)] in it.
More [foooo (5 = 100%)] and [foo (6 = 120%)] here." ]
}

@test "percentages against total for unnamed count via pattern" {
    run extractMatches --percentage-total fo+ 5 --count fo+ --global <<-'EOF'
This has foo, foo and foofoo in it.
More foooo and foo here.
EOF
    [ "$output" = "This has [foo (1 = 20%)], [foo (2 = 40%)] and [foo (3 = 60%)][foo (4 = 80%)] in it.
...
@test "percentages against total for three counts from a single match-count" {
    run extractMatches --match-count fo+ --global --name foos --percentage-total foos 5 <<-'EOF'
Or some sexy text.
...
@test "percentages against total for counts from one name, but not from another name" {
    run extractMatches --percentage-total fours 5 --match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foo text here.
Or fooo text.
EOF
    [ "$output" = "Or [some (1 = 20%)] [sexy (1 = 20%)] [text (1 = 20%)].
...
@test "percentages against total for counts from multiple names" {
    run extractMatches --percentage-total fours 5 --percentage-total foos 20 --match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foo text here.
...
@test "percentages against total for selected match-counts from multiple names" {
    run extractMatches --percentage-total fours-text 5 --percentage-total foos-fooo 50 --match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foo text here.
Or fooo text.
EOF
    [ "$output" = "Or [some (1)] [sexy (1)] [text (1 = 20%)].
...
@test "percentages against total for count and match-counts are combined under same name prefix" {
    run extractMatches --percentage-total fo 50 --count fo+ --global --name foos --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
...
Or fooo text.
EOF
    [ "$output" = "Or [some (1 = 2.0%)] [sexy (1 = 2.0%)] [text (1 = 2.0%)].
...
@test "mixed percentages for counts from multiple names against totals / counts" {
    run extractMatches --percentage-total fours 500 --name-percentages foos --match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foo text here.
Or fooo text.
EOF
    [ "$output" = "Or [some (1 = 0.2%)] [sexy (1 = 0.2%)] [text (1 = 0.2%)].
