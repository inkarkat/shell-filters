...
@test "pattern percentages for three counts from a single match-count" {
    run -0 extractMatches --match-count fo+ --global --name foos --name-percentages /fo+s/ <<-'EOF'
Or some sexy text.
...
@test "pattern percentages for counts from one name, but not from another name" {
    run -0 extractMatches --name-percentages /ours-/ --match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foo text here.
...
@test "pattern percentages for counts from multiple names combined in a single pattern" {
    run -0 extractMatches --name-percentages '/^fo(o|ur)s/' --match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foo text here.
...
And [more (2 = 17%)] [foo (4 = 31%)] [text (4 = 29%)] [here (1 = 6.7%)].
Or [fooo (1 = 6.2%)] [text (5 = 29%)].
EOF
}

@test "pattern percentages for selected match-counts from multiple names" {
    run -0 extractMatches --name-percentages '/^fours-[Tt]/' --name-percentages foos-fooo --match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foo text here.
...
@test "pattern percentages for count and match-counts are combined under same name prefix" {
    run -0 extractMatches --name-percentages '/^fo(o|ur)s/' --count fo+ --global --name foos --name foos --match-count '\<\w{4}\>' --global --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
...
@test "pattern percentages for count and match-counts with reset are combined" {
    run -0 extractMatches --name-percentages '/^fo(o|ur)s/' --count fo+ --global --name foos --reset-name '[Aa]nd' --name foos --match-count '\<\w{4}\>' --global --name fours --reset-name '[Oo]r' --name fours <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
All more simple text.
And more foooo text here.
