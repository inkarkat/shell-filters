...
@test "single match-counts in a line are flashed" {
    run -0 extractMatches --unbuffered --to flash --match-count fo+ <<-'EOF'
Just some text.
This has foo in it.
All simple lines.
More foooo here.
...
@test "two different match-counts in a line are flashed" {
    run -0 extractMatches --unbuffered --to flash --match-count fo+ --global --match-count '\<\w{4}\>' --global <<-'EOF'
Just some sexy text.
...
EOF
    assert_output - <<EOF
${R}Just (1)${N}Just ${R}some (1)${N}some ${R}sexy (1)${N}sexy ${R}text (1)${N}text.${EOL}
