...
@test "percentage threshold for counts from a single match-count omits lower ones" {
    run -0 extractMatches --match-count fo+ --global --name foos --name-percentage-threshold 16 --name-percentages foos <<-'EOF'
Or some sexy text.
This text has foo, or foooo and foofoo in it.
Let's foo more for foo and foo.
Introduce fooo here.
...
@test "same percentage threshold for counts from multiple names" {
    run -0 extractMatches --name-percentage-threshold 40 --name-percentages fours --name-percentages foos --name-percentages caps \
...
EOF
    ...
    assert_output - <<'EOF'
[LISTEN (1 = 100%)] Or [some (1 = 100%)] [sexy (1 = 50%)] text [IMPORTANT (1 = 50%)].
...
@test "different percentage thresholds for counts from multiple names" {
    run -0 extractMatches --name-percentages fours --name-percentage-threshold 40 --name-percentages foos --name-percentage-threshold 70 --name-percentages caps \
	--match-count fo+ --global --name foos --match-count '\<\w{4}\>' --global --name fours --match-count '[A-Z]{2,}' --global --name caps <<-'EOF'
LISTEN Or some sexy text IMPORTANT.
This text has foo, or foooo and foofoo in it.
LISTEN All more simple text.
...
EOF
    ...
    assert_output - <<'EOF'
[LISTEN (1 = 100%)] Or [some (1 = 100%)] [sexy (1 = 50%)] [text (1 = 33%)] IMPORTANT.
