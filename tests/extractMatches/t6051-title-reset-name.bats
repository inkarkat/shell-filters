...
@test "match resets by name are shown in title" {
    ...
    run -0 extractMatches --to title --reset-name All --name fours --regexp '\<\w{4}\>' --name fours <<<"$SIMPLE_INPUT"
    assert_output - <<EOF
Just some text.
...
@test "count resets by name are shown in title" {
    ...
    run -0 extractMatches --to title --reset-name All --name fours --count '\<\w{4}\>' --name fours <<<"$SIMPLE_INPUT"
    assert_output - <<EOF
Just some text.
...
@test "match-count resets by name are shown in title" {
    ...
    run -0 extractMatches --to title --reset-name All --name fours --match-count '\<\w{4}\>' --name fours <<<"$SIMPLE_INPUT"
    assert_output - <<EOF
Just some text.
${R}Just:1${N}This has foo in it.
