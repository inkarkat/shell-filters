...
@test "match resets are notify-sent every 3 lines and at the end" {
    run -0 extractMatches --to notify --regexp 'foo[0-9]+' --reset 'All|Your' <<<"$DELAY_INPUT"
...
@test "count resets are notify-sent every 3 lines and at the end" {
    run -0 extractMatches --to notify --count 'foo[24-9]+' --reset 'All|Your' <<<"$DELAY_INPUT"
    assert_output "$DELAY_INPUT"
...
@test "match-count resets are notify-sent every 3 lines and at the end" {
    run -0 extractMatches --to notify --match-count '\<\w{4}\>' --reset 'All|Rex' <<<"$DELAY_INPUT"
    assert_output "$DELAY_INPUT"
